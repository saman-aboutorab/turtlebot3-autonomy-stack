...
**RPi4 directory layout:**
```
~/ros2_ws/                                  ← colcon workspace root
    src/
        turtlebot3-autonomy-stack/          ← git repo lives here
            tb3_bringup/
            tb3_odometry/
            tb3_navigation/
            hardware/
    ...
    log/                                    ← build logs (not in git)
```

To pull updates and rebuild after a `git push` from the laptop:
```bash
cd ~/ros2_ws/src/turtlebot3-autonomy-stack && git pull origin stage-1
cd ~/ros2_ws && colcon build --symlink-install --packages-select tb3_bringup
